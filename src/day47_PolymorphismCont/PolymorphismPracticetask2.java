...
public class PolymorphismPracticetask2 {
    public static void main(String[] args) {
        ...
        for (Employee TestAndDevscrumMember : employees) {
            if (TestAndDevscrumMember instanceof Tester || TestAndDevscrumMember instanceof Developer){
                scrumMembers.add(TestAndDevscrumMember);
            }
...
Tester maxSalTester=testers.get(0);
        Developer maxSalDev=developers.get(0);
        for (Tester tester : testers) {
            if (tester.getSalary()>maxSalTester.getSalary()){
                maxSalTester=tester;
            }
        }
        for (Developer developer : developers) {
            if (developer.getSalary()>maxSalDev.getSalary()){maxSalDev=developer;
            }

        }
        System.out.println(maxSalTester);
        System.out.println("maxSalDev = " + maxSalDev);
